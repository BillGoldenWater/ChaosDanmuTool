...
impl ConfigManager {
  ...
  pub async fn tick(&mut self) {
    ...
    if a_lock("cm_cfg", &self.config)
      ...
      .save_on_change
    {
      if *a_lock("cm_changed", &self.changed).await
        && a_lock("cm_lSTs", &self.last_save_ts)
          .await
          .elapsed()
          .as_secs()
          >= 5
...
macro_rules! get_cfg {
  () => {{
    use static_object::StaticObject;
    $crate::app::config_manager::ConfigManager::i()
      .get_readonly_config()
      .await
  }};
  (sync) => {{
    use static_object::StaticObject;
    $crate::utils::async_utils::run_blocking(
      $crate::app::config_manager::ConfigManager::i().get_readonly_config(),
    )
  }};
}
