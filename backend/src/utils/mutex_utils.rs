...
pub fn lock_custom_timeout<T>(mutex: &Mutex<T>, time_millis: u64) -> MutexGuard<T> {
  run_blocking(a_lock_custom_timeout(mutex, time_millis))
}

pub async fn a_lock<T>(mutex: &Mutex<T>) -> MutexGuard<T> {
  a_lock_custom_timeout(mutex, *TIMEOUT_MILLIS).await
}

pub async fn a_lock_custom_timeout<T>(
  mutex: &Mutex<T>,
  time_millis: u64,
) -> MutexGuard<T> {
  loop {
    ...
    if let Ok(result) = lock_result {
      ...
      error!("failed to get lock in {time_millis}ms");
      print_trace();
      continue;
    }
  }
