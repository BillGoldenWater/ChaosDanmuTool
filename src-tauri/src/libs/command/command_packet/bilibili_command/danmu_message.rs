...
impl DanmuMessage {
  ...
  fn parse_meta(&mut self, meta: &Value) {
    ...
    self.fontsize = meta.get(2).unwrap_or(&Value::Null).as_i64().unwrap_or(0) as i32;
    self.color = meta.get(3).unwrap_or(&Value::Null).as_i64().unwrap_or(0) as i32;
    self.timestamp = meta.get(4).unwrap_or(&Value::Null).as_u64().unwrap_or(0);
    self.danmu_type =
      DanmuType::from_u32(meta.get(12).unwrap_or(&Value::Null).as_u64().unwrap_or(0) as u32);
  ...
  fn parse_user_data(&mut self, user_data: &Value) {
    ...
    self.uid = user_data
      ...
      .as_u64()
      .unwrap_or(0);
    self.u_name = user_data
      .get(1)
      .unwrap_or(&Value::Null)
      .as_str()
      .unwrap_or("[CDT-Default]")
      .to_string();
    self.is_manager = !user_data
      .get(2)
      .unwrap_or(&Value::Null)
      .as_u64()
      .unwrap_or(0)
      .eq(&0);
    self.is_vip = !user_data
      .get(3)
      .unwrap_or(&Value::Null)
      .as_u64()
      .unwrap_or(0)
      .eq(&0);
    self.is_svip = !user_data
      .get(4)
      .unwrap_or(&Value::Null)
      .as_u64()
      .unwrap_or(0)
